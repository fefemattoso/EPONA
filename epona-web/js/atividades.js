document.addEventListener('DOMContentLoaded', () => {  
    ...
    async function carregarAtividades() {  
        ...
        await checarDados(); // Aguarda a checagem de dados  
        try {  
    ...
    function deletarAtividade(id) {  
        fetch(`http://localhost:3000/atividade/${id}`, {  
            ...
            .catch(e => console.error("Erro ao excluir atividade:", e));  
    }  
    ...
    editAtividadeForm.addEventListener('submit', (e) => {  
        e.preventDefault();  
        const titulo = document.getElementById('newTitulo').value;  
        const descricao = document.getElementById('newDescricao').value;  
  
        editarAtividade(atividadeIdParaEditar, titulo, descricao);  
    });  
  
    async function editarAtividade(id, titulo, descricao) {  
        try {  
...
//Abrir confirmação de exclusão de atividade
const editarAtividadeModal = document.getElementById('editAtividadeModal');
...
async function editarAtividade(id){
    ...
    try{
        let atividade = await fetch(`http://localhost:3000/atividade/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        let atividadeJson = await atividade.json()
        let titulo = atividadeJson.titulo
        let descricao = atividadeJson.descricao
        let response = await fetch(`http://localhost:3000/atividade/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (response.ok) {
            let result = await response.json();
            console.log(result); // Mensagem de sucesso
        }
    } catch (e) {
        console.error("Erro ao editar atividade:", e)
    }
